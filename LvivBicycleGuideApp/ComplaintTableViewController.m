...
-(void)viewWillAppear:(BOOL)animated
{
    ...
    testArray = [all[0][@"comment"]mutableCopy];
    [self.tableView reloadData];
//    [query findObjectsInBackgroundWithBlock:^(NSArray *objects, NSError *error) {
//        testArray = [objects[0][@"comment"] mutableCopy];
//        [self.tableView reloadData];
//    }];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    cell.likeImage.image = [[temp valueForKey:@"likeDislike"] isEqual:@1 ] ? [UIImage imageNamed:@"like.png"] : [UIImage imageNamed:@"dislike.png"];  //temp.likeDislike == YES  ? [UIImage imageNamed:@"like.png"] : [UIImage imageNamed:@"dislike.png"];
